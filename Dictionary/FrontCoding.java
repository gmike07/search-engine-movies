...
public class FrontCoding extends ConcatenatedStringDict
{
    ...
    public void append(String last_word)
    {
        try {
            ...
            {
                byte[] bytes =
                        tools.index_tools.int_to_byte(helper_string_writer.getBytePointer() - curr_pointer);
                lengths.add(bytes);
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    {
        int block = i / K;
        int ptr_ = tools.index_tools. byte_to_int(ptr_dict, block * tools.index_tools.INT_SIZE);
        ...
        for(int offset_helper = 0; offset_helper < i % K; offset_helper++)
        {
            ...
            ptr_ += tools.index_tools.byte_to_int(arr);
        }
        return ptr_;
    }
    ...
    public void save_to_disk()
    {
        if (saved)
            return;
        ptr_dict = new byte[ptr.size() * tools.index_tools.INT_SIZE];
        byte[] current_int = new byte[tools.index_tools.INT_SIZE];
        for (int i = 0; i < ptr.size(); i++)
        {
            tools.index_tools.int_to_byte(current_int, ptr.get(i));
            for (int j = 0; j < tools.index_tools.INT_SIZE; j++)
                ptr_dict[j + i * tools.index_tools.INT_SIZE] = current_int[j];
        }
        ...
        // save metadata
        try {
            tools.index_tools.int_to_byte(current_int, ptr.size());
            meta_data_rw = new RandomAccessFile(path_meta_data, "rw");
            meta_data_rw.write(current_int);
            tools.index_tools.int_to_byte(current_int, lengths.size() * BYTE_LENGTH_SIZE);
            meta_data_rw.write(current_int);
            tools.index_tools.int_to_byte(current_int, lengths.size() * BYTE_PREFIX_SIZE);
            meta_data_rw.write(current_int);
            meta_data_rw.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void load_from_disk()
    {
        if (loaded)
            return;
        // load metadata
        byte[] num_rows_temp = new byte[tools.index_tools.INT_SIZE];
        byte[] num_rows_temp2 = new byte[tools.index_tools.INT_SIZE];
        byte[] num_rows_temp3 = new byte[tools.index_tools.INT_SIZE];
        try {
            meta_data_rw = new RandomAccessFile(path_meta_data, "rw");
        ...
        int num_rows_ptr = tools.index_tools.byte_to_int(num_rows_temp);
        ptr_dict = new byte[num_rows_ptr * index_tools.INT_SIZE];
        int num_rows_lengths = tools.index_tools.byte_to_int(num_rows_temp2);
        length_dict = new byte[num_rows_lengths];
        int num_rows_prefixes = tools.index_tools.byte_to_int(num_rows_temp3);
