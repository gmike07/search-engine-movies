...
public class IndexReader
{
    ...
    {
        byte[] current_list = pid_ind.get_synced_dict_field_by_dict("pid", "pid_ids_list", productId.getBytes(StandardCharsets.UTF_8));
        Vector<Integer> ans = new Vector<>();
        if (current_list == null)
            return ans.elements();

        int first_id = tools.index_tools.byte_to_int(current_list, 0);
        int num_of_rev = tools.index_tools.byte_to_int(current_list, INT_SIZE);
