...
//        RandomAccessFile reader;
    ...
    private void decodeData(byte[] bytes, int row_size)
    {
        ...
        try{
            lst = integerCompression.decodeList(index_rw, 0, (int)index_rw.length());
        }catch (IOException e)
        {
            e.printStackTrace();
            return;
        ...
        for (Integer integer : lst) {
            byte[] num = index_tools.int_to_byte(integer - 1);
            int fieldSize = indexes.get(currField + 1) - indexes.get(currField);
            if (fieldSize >= 0) System.arraycopy(num, 0, bytes, currentIndex, fieldSize);
            currentIndex += fieldSize;
            currField = (currField + 1) % (indexes.size() - 1);
        }
    }
}
